from market_data import MarketDeposit,  MarketFuture, MarketSwap, MarketBond
...
def price_bond(bond: MarketBond, discount_curve: dict) -> float:
     return sum(bond.cashflows[i] * discount_curve.evaluate(bond.coupon_dates[i]) for i in range(len(bond.coupon_dates))) 